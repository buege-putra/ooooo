...
from src.captioning.lstm.layers import LSTMCell
...
class LSTMDecoder(BaseCaptionDecoder):
    """decoder caption lstm dengan strategi pre-inject"""
    ...
    def __init__(
        ...
        super().__init__(name=name or "LSTMDecoder")
        ...
        self.cells = [LSTMCell(hidden_size, name=f"lstm_{index + 1}") for index in range(self.recurrent_layers)]
    ...
    def _run_cells(self, inputs: np.ndarray, state: list[LSTMState]) -> tuple[np.ndarray, list[LSTMState]]:
        output = inputs
        next_state: list[LSTMState] = []
        for index, cell in enumerate(self.cells):
            output, cell_state = cell.step(output, state[index])
            next_state.append(cell_state)
        return output, next_state

    def _inject_image(self, image_features: np.ndarray, state: list[LSTMState]) -> list[LSTMState]:
    ...
    def load_keras_weights(self, keras_model: Any) -> None:
        ...
        for index, cell in enumerate(self.cells):
            cell.set_weights(keras_model.get_layer(f"lstm_{index + 1}").get_weights())
