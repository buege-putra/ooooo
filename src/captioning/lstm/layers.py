...
class LSTMCell(RecurrentCell):
    ...
    def build(self, input_dim: int) -> None:
        ...
        self.kernel = np.zeros((self.input_dim, 4 * self.units), dtype=self.dtype)
        self.recurrent_kernel = np.zeros((self.units, 4 * self.units), dtype=self.dtype)
        self.bias = np.zeros((4 * self.units,), dtype=self.dtype)

    def initial_state(self, batch_size: int) -> tuple[np.ndarray, np.ndarray]:
        h = np.zeros((batch_size, self.units), dtype=self.dtype)
        c = np.zeros((batch_size, self.units), dtype=self.dtype)
        return h, c

    def step(self, inputs: np.ndarray, state: tuple[np.ndarray, np.ndarray]) -> tuple[np.ndarray, tuple[np.ndarray, np.ndarray]]:
        ...
        h_prev, c_prev = state
        z = np.asarray(inputs, dtype=self.dtype) @ self.kernel + h_prev @ self.recurrent_kernel + self.bias
    ...
    def set_weights(self, weights: Sequence[np.ndarray]) -> None:
        ...
        expected_kernel = (kernel.shape[0], 4 * self.units) if self.kernel is None else self.kernel.shape
        validate_weight_shape(self.name, kernel.shape, expected_kernel, "kernel")
        validate_weight_shape(self.name, recurrent_kernel.shape, (self.units, 4 * self.units), "recurrent_kernel")
        validate_weight_shape(self.name, bias.shape, (4 * self.units,), "bias")
